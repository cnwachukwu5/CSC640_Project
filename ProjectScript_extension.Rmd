...
# Compute activity time and continuous working time
...
data_activityevent2$continuousrate <- round((data_activityevent2$continuousrate),0)

data_activeTime3 <- getActiveTime_df(data_activeTime)
```

```{r}
data_activityevent2 <- data_activityevent2 %>% mutate_if(is.factor, as.character) %>% mutate_if(is.double, as.integer) 
data_activityevent2
```

```{r}
data_activeTime_lm <- lm(TotalActivityTimeInHours ~ day, data = data_activityevent2)
