...
## Data Collection
It was a challenge to get the exact data used in the paper. We contacted the authors but they directed us to the KAVE interaction dataset of MSR challenge 2018. The Interaction dataset consists of JSON files representing recorded events on a user's IDE (Visual studio).KAVE has FeedBag++ plugin on IDEs for capturing events related to developers activities on the IDE. This dataset has about 11.1 million records. KAVE project provides a Java program to retrieve the various events recorded while a developer works on the IDE. We extended this Java program to pull the three events and related fields used in the paper. We hosted the modified project and the extracted data in csv files on our project github page, [CSC640_Project](https://github.com/cnwachukwu5/CSC640_Project/tree/master/java-cc-kave-examples/)


## Replication

### Research question 1: 
Does day of the week affect development time and efficiency? This is to identify correlation between day of the week and development efficiency.
...
Alternate Hypothesis (Ha): Day of the week does affect development time and efficiency

Our analysis of the sampled data showed that:

### Research question 2:
Is there a relationship between development efficient and working time within a 24-hour period? 

#### Hypothesis
Formulate an appropriate null hypothesis.
Null Hypothesis (Ho): There is no relationship between development efficiency and working time within a 24-hour period

Alternate Hypothesis (Ha): There is relationship between development efficiency and working time within a 24-hour period

Our analysis of the sampled data showed that:

### Research question 3:
Does continuous working time have an effect on development efficiency?

#### Hypothesis
Formulate an appropriate null hypothesis.
Null Hypothesis (Ho): Continous working time does not affect development efficiency

Alternate Hypothesis (Ha): Continous working time does affect development efficiency

Our analysis of the sampled data showed that:

```{r}
# Get week day from TriggeredAT field and compute: activitytime, totalworkingtime and continuous working time
...
```{r}
ggplot(data_activityevent2, aes(x = day, y = totalActivityTimeInHours)) + geom_bar(stat = "identity")

View(data_activityevent2)
```


#Using ggplot to plot the total working duration vs the day of the week
```{r}
ggplot(data_activityevent2, aes(x = day, y = continuousWorkingTimeInHours, TotalActivityTimeInHours)) + geom_bar(stat = "identity")
```
